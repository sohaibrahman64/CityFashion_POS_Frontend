  ...
  useEffect(() => {
    ...
    if (editProductData) {
      try {
        ...
        if (product.stock?.openingQuantity)
          setOpeningQuantity(product.stock.openingQuantity.toString());
        if (product.stock?.atPrice)
          setAtPrice(product.stock.atPrice.toString());
        if (product.stock?.asOfDate) setAsOfDate(product.stock.asOfDate);
        if (product.stock?.minStock)
          setMinStock(product.stock.minStock.toString());
        ...
        if (product.purchasePriceType)
          setPurchasePriceType(product.purchasePriceType);
    ...
    try {
      ...
      const productData = {
        ...
        purchasePriceTaxes: {
          ...
          purchasePriceType,
          taxRate: selectedTax
            ? taxRates.find((rate) => rate.id == selectedTax)
            : null,
        },
      };
      ...
      if (isEditing) {
        ...
        response = await axios.put(
          `${BASE_URL}/${apiEndpoint}/${editingProductId}`,
          formData,
          {
            headers: {
      ...
      console.error(
        isEditing ? "Error updating product:" : "Error saving product:",
        error
      );
      alert(
        isEditing
          ? "Error updating product. Please check your connection and try again."
          : "Error saving product. Please check your connection and try again."
      );
    } finally {
      setLoading(false);
    }
        ...
        <div className="product-inputs-group">
          ...
          <div className="form-group-product">
            ...
            <div className="input-with-icon">
              ...
              />
              <span className="search-icon" onClick={handleSearchHSN}>
                🔍
              </span>
            </div>
          </div>
