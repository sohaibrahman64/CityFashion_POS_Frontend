...
const AddSalesOrder = () => {
  ...
  const [
    isFromSalesOrderDashboard,
    setIsFromSalesOrderDashboard,
  ] = useState(false);
  ...
  const [salesOrderNumber, setSalesOrderNumber] = useState("ORD-00001");
  ...
  const shareViaWhatsApp = () => {
    try {
      ...
      const message = `Sales Order Details:
...
Thank you for your business!`;
  ...
  const generateAndDownloadPDF = async () => {
    try {
      if (!salesOrderPreviewRef.current) {
        console.error("Sales Order preview element not found");
        return;
      }
      ...
      const opt = {
        margin: [10, 10, 10, 10],
        filename: `Sales_Order_${partyName || "Customer"}_${
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .add-sales-order-preview { max-width: 800px; margin: 0 auto; }
            table { width: 100%; border-collapse: collapse; margin: 10px 0; }
            th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }
            th { background-color: #f2f2f2; }
            .add-sales-order-header { text-align: center; margin-bottom: 20px; }
            .add-sales-order-title { text-align: center; color: #333; }
            .add-sales-order-details { margin: 20px 0; }
            .add-sales-order-summary-item { display: flex; justify-content: space-between; margin: 5px 0; }
            .add-sales-order-total-row { font-weight: bold; background-color: #f9f9f9; }
            @media print {
              body { margin: 0; }
              .add-sales-order-no-print { display: none; }
            }
          </style>
        </head>
        <body>
  ...
  const createSalesOrderTransaction = async (salesOrderData) => {
    try {
      ...
      const payload = {
        salesOrderId: salesOrderData.salesOrderId || null,
        salesOrderNumber:
          salesOrderData.salesOrderNumber || salesOrderNumber,
        partyName: partyName,
        partyId: partyId,
        ...
        dueDate: dueDate,
        orderStatus: "OVERDUE",
        advanceAmount: salesOrderData.advanceAmount || advanceAmount,
        balanceAmount: calculateSubTotal() - parseFloat(salesOrderData.advanceAmount || advanceAmount),
        taxAmount: salesOrderData.totalTaxAmount || 0.0,
        ...
        createdBy: "SYSTEM",
        notes: `Sales Order Data | Sales Order: ${
          salesOrderData.salesOrderNumber || salesOrderNumber
        }`,
      ...
      const res = await fetch(
        `${BASE_URL}/${CREATE_SALES_ORDER_TRANSACTION}`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        ...
        console.error("Failed to create sales order transaction", txt);
      }
    } catch (err) {
      console.error("Error creating sales order transaction:", err);
    }
  };


  const incrementSalesOrderNumber = (currentSalesOrderNumber) => {
    // Handle alphanumeric estimate numbers like "ORD-00012"
    const match = currentSalesOrderNumber.match(/^([A-Z]+)-(\d+)$/);
    ...
    if (!isNaN(currentSalesOrderNumber)) {
      return parseInt(currentSalesOrderNumber, 10) + 1;
    }
  ...
  const handleSaveNew = async () => {
    ...
    try {
      const payload = {
        ...
        dueDate: dueDate,
        totalAmount: calculateSubTotal(),
        balanceAmount: calculateSubTotal() - parseFloat(advanceAmount || 0),
      ...
      const response = await fetch(
        `${BASE_URL}/${CREATE_NEW_SALES_ORDER}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        setToastMessage("Sales Order created successfully!");
        setToastType("success");
        setShowToast(true);

        // Also create sales order transaction
        await createSalesOrderTransaction(data);

        // Clear all form fields after successful sales order creation
        setPartyName("");
        setPartyPhone("");
        setPartyId(null);
        setAdvanceAmount("");
        ...
        setSalesOrderNumber((prevNumber) => incrementSalesOrderNumber(prevNumber));
        ...
        setTimeout(() => {
          routerNavigate("/sales/order/preview", {
            state: { salesOrderData: data },
          });
        }, 2500);
        // Add any additional logic here, like redirecting to the estimate page
      } else {
        console.error("Failed to create sales order");
        setToastMessage("Failed to create sales order. Please try again.");
        setToastType("error");
        setShowToast(true);
      }
    } catch (error) {
      ...
      setToastMessage(
        "Error creating sales order. Please check your connection and try again."
      );
      setToastType("error");
      setShowToast(true);
    }
  ...
  const fetchSalesOrderNumber = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}/${GENERATE_SALES_ORDER_NUMBER}`
      );
      ...
      if (response.ok) {
        ...
        console.error("Failed to fetch sales order number");
        setSalesOrderNumber("ORD-00001");
      }
    } catch (error) {
  ...
  const handleFullyReceivedChange = (checked) => {
    ...
    if (checked) {
      ...
      setAdvanceAmount(calculateSubTotal().toFixed(2));
    } else {
  ...
  return (
    <div className="add-sales-order-container">
      {/* Header Section */}
      <div className="add-sales-order-header-section">
        <div className="add-sales-order-header-section-left">
          <span className="add-sales-order-label">
            Add Sales Order
          </span>
        </div>
        <div className="add-sales-order-header-section-right">
          <button
            className="add-sales-order-btn-back"
            onClick={() => {
      ...
      <div className="main-content">
        ...
        <div className="add-sales-order-left-column">
          <div className="add-sales-order-input-section">
            <div className="add-sales-order-customer-fields">
              <div className="add-sales-order-input-group">
            ...
            <div className="add-sales-order-address-fields">
              {showBillingAddress && (
                <div className="add-sales-order-address-input-group">
                  ...
                  <textarea
                    ...
                    ref={billingRef}
                  />
                  <div className="add-sales-order-address-actions">
                    <button
                      type="button"
                      className="add-sales-order-address-btn remove"
                    ...
                    <button
                      type="button"
                      className="add-sales-order-address-btn change"
            ...
            <div className="add-sales-order-date-fields">
              <div className="add-sales-order-input-group">
            ...
            <div className="add-sales-order-items-section">
              <table className="add-sales-order-items-table">
                <thead>
                  <tr>
                    <th className="add-sales-order-header-cell">#</th>
                    <th className="add-sales-order-header-cell">ITEM</th>
                    <th className="add-sales-order-header-cell">QTY</th>
                    <th className="add-sales-order-header-cell add-sales-order-price-unit-header">
                      <div className="add-sales-order-price-unit-split">
                        <div className="add-sales-order-price-label">
                          PRICE/UNIT
                        </div>
                        <div className="add-sales-order-tax-type-select">
                          <select
                            className="add-sales-order-header-tax-select"
                            value={headerTaxType}
                            onChange={(e) =>
                    ...
                    <th className="add-sales-order-header-cell add-sales-order-discount-header">
                      <div className="add-sales-order-discount-split">
                        <div className="add-sales-order-discount-label">
                          DISCOUNT (%)
                        </div>
                        <div className="add-sales-order-discount-sub-split">
                          <div className="add-sales-order-discount-percent-label">
                            %
                          </div>
                          <div className="add-sales-order-discount-amount-label">
                            AMOUNT
                          </div>
                        </div>
                      </div>
                    </th>
                    <th className="add-sales-order-header-cell add-sales-order-tax-header">
                      <div className="add-sales-order-tax-split">
                        <div className="add-sales-order-tax-label">
                          TAX
                        </div>
                        <div className="add-sales-order-tax-sub-split">
                          <div className="add-sales-order-tax-percent-label">
                            %
                          </div>
                          <div className="add-sales-order-tax-amount-label">
                            AMOUNT
                          </div>
                        </div>
                      </div>
                    </th>
                    <th className="add-sales-order-header-cell">
                      TOTAL
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {itemInputs.map((item, index) => (
                    <tr
                      key={item.id}
                      className="add-sales-order-item-row"
                    >
                      <td className="add-sales-order-cell add-sales-order-item-id-cell">
                        <span className="add-sales-order-item-id">
                          {item.id}
                        </span>
                        {(itemInputs.length > 1 || index === 0) && (
                          <button
                            className="add-sales-order-remove-row-btn"
                      ...
                      <td className="add-sales-order-cell add-sales-order-item-name-cell">
                        <ItemsDropdown
                          value={item.itemName}
                          onChange={(value) =>
                      ...
                      <td className="add-sales-order-cell">
                        <input
                          type="number"
                          placeholder="0"
                          value={item.quantity}
                      ...
                      <td className="add-sales-order-cell add-sales-order-price-unit-cell">
                        <input
                          type="number"
                          placeholder="0.00"
                          value={item.price}
                          ...
                          className="add-sales-order-price-input"
                        />
                      </td>
                      <td className="add-sales-order-cell add-sales-order-discount-cell">
                        <div className="add-sales-order-discount-split">
                          <div className="add-sales-order-discount-sub-split">
                            <input
                              type="number"
                              ...
                              className="add-sales-order-discount-input"
                            />
                            <input
                              type="number"
                              placeholder="0.00"
                              ...
                              className="add-sales-order-discount-amount-input"
                            />
                          </div>
                        </div>
                      </td>
                      <td className="add-sales-order-cell add-sales-order-tax-cell">
                        <div className="add-sales-order-tax-split">
                          <div className="add-sales-order-tax-sub-split">
                            <select
                              ...
                              className="add-sales-order-tax-select"
                            >
                            ...
                            <input
                              ...
                              readOnly
                              className="add-sales-order-tax-amount-input"
                            />
                          </div>
                        </div>
                      </td>
              ...
              <div
                className="add-sales-order-add-row-btn"
                onClick={addNewRow}
              >
                + Add Row
              </div>
            </div>
            <div className="add-sales-order-payment-section">
              <div className="add-sales-order-summary-row">
                <div className="add-sales-order-summary-item">
                  <span>Sub Total</span>
                  <span>
              ...
              <div className="add-sales-order-payment-row">
                <div className="add-sales-order-payment-label">
                  <label>Advance Amount</label>
                </div>
                <div className="add-sales-order-payment-controls">
                  <input
                    type="number"
                    placeholder="0.00"
                    value={advanceAmount}
                  ...
                  />
                  <div className="add-sales-order-checkbox-group">
                    <input
                      type="checkbox"
                      id="fullyReceived"
              ...
              <div className="add-sales-order-balance-row">
                <div className="add-sales-order-summary-item">
                  <span>Balance:</span>
                  <span>
                    ₹{" "}
                    {formatNumberWithCommas(
                      (
                        calculateSubTotal() - parseFloat(advanceAmount || 0)
                      ).toFixed(2)
                    )}
                  </span>
                </div>
          ...
          <div className="add-sales-order-total-amount-bar">
        ...
        <div className="add-sales-order-right-column">
          <div
            className="add-sales-order-estimate-preview"
            ref={salesOrderPreviewRef}
          >
            <h2 className="add-sales-order-estimate-title">
              Sales Order
            </h2>

            <div className="add-sales-order-estimate-details">
              <div className="add-sales-order-details-header">
                <div className="add-sales-order-estimate-for">
                  <h4>Sales Order For</h4>
                  <p>{partyName || "Customer Name"}</p>
                  <p>{partyPhone || "Contact No. "}</p>
                </div>

                <div className="add-sales-order-estimate-info">
                  <h4>Sales Order Details</h4>
                  <div className="add-sales-order-info-row">
                    <span>Ref. No. {salesOrderNumber}</span>
                  </div>
                  <div className="add-sales-order-info-row">
                    <span>Order Date: {orderDate ? new Date(orderDate).toLocaleDateString() : new Date().toLocaleDateString()}</span>
                  </div>
                  <div className="add-sales-order-info-row">
                    <span>Due Date: {dueDate ? new Date(dueDate).toLocaleDateString() : new Date().toLocaleDateString()}</span>
                  </div>
                </div>
              </div>
              {/* Items Table - Only visible when items are selected */}
              {itemInputs.some((item) => item.itemName.trim() !== "") && (
                <div className="add-sales-order-estimate-items-table">
                  <table className="add-sales-order-items-preview-table">
                    <thead>
                      <tr>
                        <th>#</th>
                        <th>Item Name</th>
                    ...
                    <tbody>
                      {itemInputs
                        ...
                        .map((item, index) => (
                          <tr
                            key={item.id}
                            className="add-sales-order-estimate-item-preview-row"
                          >
                            <td>{index + 1}</td>
                            <td>{item.itemName}</td>
                            ...
                            <td>
                              ...
                              <span className="add-sales-order-discount-percentage">
                                ({item.discount || 0}%)
                              </span>
                            </td>
                            <td>
                              ...
                              <span className="add-sales-order-gst-percentage">
                                (
                      ...
                      {/* Total Row */}
                      <tr className="add-sales-order-total-row">
                        <td colSpan="2">
                          <strong>Total</strong>
                        </td>
                        <td>
                    ...
                    <tfoot>
                      <tr>
                        <td
                          colSpan="7"
                          className="add-sales-order-no-padding-cell"
                        >
                          <div className="add-sales-order-tax-summary-wrapper">
                            <div className="add-sales-order-tax-summary-title">
                              Tax Summary:
                            </div>
                            <table className="add-sales-order-tax-summary-table">
                              <thead>
                                {(() => {
                                  ...
                                  return (
                                    <>
                                      <tr>
                                        <th
                                          rowSpan="2"
                                          className="add-sales-order-ts-col-hsn"
                                        >
                                          HSN/ SAC
                                        </th>
                                        <th
                                          rowSpan="2"
                                          className="add-sales-order-ts-col-taxable"
                                        >
                                          Taxable amount (₹)
                                        </th>
                                        {hasIGST ? (
                                          <th
                                            colSpan="2"
                                            className="add-sales-order-ts-col-igst"
                                          >
                                            IGST
                                          </th>
                                        ) : (
                                          <>
                                            <th
                                              colSpan="2"
                                              className="add-sales-order-ts-col-cgst"
                                            >
                                              CGST
                                            </th>
                                            <th
                                              colSpan="2"
                                              className="add-sales-order-ts-col-sgst"
                                            >
                                              SGST
                                            </th>
                                          </>
                                        ...
                                        <th
                                          rowSpan="2"
                                          className="add-sales-order-ts-col-total"
                                        >
                                          Total Tax (₹)
                                        </th>
                                      </tr>
                                      <tr>
                                        {hasIGST ? (
                                          <>
                                            <th className="add-sales-order-ts-col-rate">
                                              Rate (%)
                                            </th>
                                            <th className="add-sales-order-ts-col-amt">
                                              Amt (₹)
                                            </th>
                                          </>
                                        ) : (
                                      ...
                                      {taxGroupsArray.map((group, index) => {
                                        if (group.isIGST) {
                                          ...
                                          return (
                                            <tr key={index}>
                                              <td></td>
                                              <td className="add-sales-order-ta-right">
                                                {formatNumberWithCommas(
                                                  group.taxableAmount.toFixed(2)
                                                )}
                                              </td>
                                              <td className="add-sales-order-ta-center">
              ...
              <div className="add-sales-order-estimate-bottom-section">
                <div className="add-sales-order-estimate-left-column-content">
                  <div className="add-sales-order-amount-words">
                    <p>
                      <strong>Sales Order Amount In Words: </strong>{" "}
                      {numberToWords(calculateSubTotal())}
                    </p>
                  </div>
                  <div className="add-sales-order-terms">
                    <p>
                ...
                <div className="add-sales-order-right-column-content">
                  <div className="add-sales-order-summary-item">
                    <strong>Total</strong>
                    <span>
                      ₹ {formatNumberWithCommas(calculateSubTotal().toFixed(2))}
                  ...
                  <div className="add-sales-order-summary-item add-sales-order-no-border">
                    <span className="add-sales-order-total-label">
                      <strong>Total</strong>
                    </span>
                    <span>
                      ₹ {formatNumberWithCommas(calculateSubTotal().toFixed(2))}
                  ...
                  <div className="add-sales-order-summary-item">
                    ...
                    <span>
                      ₹{" "}
                      {formatNumberWithCommas(
                        parseFloat(advanceAmount || 0).toFixed(2)
                      )}
                    </span>
                  </div>
                  <div className="add-sales-order-summary-item">
              ...
              {/* <div className="add-sales-order-footer">
                <div className="add-sales-order-company-signature">
                  <p>For : {companyName}</p>
            ...
            <div className="add-sales-order-action-buttons">
              <button
                className="add-sales-order-save-new-btn"
                onClick={handleSaveNew}
              >
                Save & New
              </button>
              <div className="add-sales-order-action-icons">
                <button
                  className="add-sales-order-icon-btn"
