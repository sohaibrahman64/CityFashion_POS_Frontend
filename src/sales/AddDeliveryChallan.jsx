...
const AddDeliveryChallan = () => {
  ...
  const [invoiceNumber, setInvoiceNumber] = useState("DC-00001");
  ...
  const [
    isFromDeliveryChallanDashboard,
    setIsFromDeliveryChallanDashboard,
  ] = useState(false);
  ...
  const [deliveryChallanNumber, setDeliveryChallanNumber] = useState("DC-00001");
  ...
  const shareViaWhatsApp = () => {
    try {
      ...
      const message = `Delivery Challan Details:
...
Thank you for your business!`;
  ...
  const generateAndDownloadPDF = async () => {
    try {
      ...
      const opt = {
        margin: [10, 10, 10, 10],
        filename: `Delivery_Challan_${partyName || "Customer"}_${
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .add-delivery-challan-preview { max-width: 800px; margin: 0 auto; }
            table { width: 100%; border-collapse: collapse; margin: 10px 0; }
            th, td { border: 1px solid #ddd; padding: 8px; text-align: left; }
            th { background-color: #f2f2f2; }
            .add-delivery-challan-header { text-align: center; margin-bottom: 20px; }
            .add-delivery-challan-title { text-align: center; color: #333; }
            .add-delivery-challan-details { margin: 20px 0; }
            .add-delivery-challan-summary-item { display: flex; justify-content: space-between; margin: 5px 0; }
            .add-delivery-challan-total-row { font-weight: bold; background-color: #f9f9f9; }
            @media print {
              body { margin: 0; }
              .add-delivery-challan-no-print { display: none; }
            }
          </style>
        </head>
        <body>
  ...
  const validateDeliveryChallanData = () => {
  ...
  const createDeliveryChallanTransaction = async (deliveryChallanData) => {
    try {
      ...
      const payload = {
        deliveryChallanId: deliveryChallanData.deliveryChallanId || null,
        deliveryChallanNumber:
          deliveryChallanData.deliveryChallanNumber || deliveryChallanNumber,
        partyName: partyName,
        partyId: partyId,
        totalAmount: calculateSubTotal(),
        balanceAmount: calculateSubTotal(),
        deliveryChallanInvoiceDate: invoiceDate,
        deliveryChallanDueDate: dueDate,
        billingAddress: billingAddress,
        taxAmount: deliveryChallanData.totalTaxAmount || 0.0,
        ...
        createdBy: "SYSTEM",
        notes: `Delivery Challan Data | Delivery Challan: ${
          deliveryChallanData.deliveryChallanNumber || deliveryChallanNumber
        }`,
      ...
      const res = await fetch(
        `${BASE_URL}/${CREATE_DELIVERY_CHALLAN_TRANSACTION}`,
        {
          method: "POST",
      ...
      if (!res.ok) {
        ...
        console.error("Failed to create delivery challan transaction", txt);
      }
    } catch (err) {
      console.error("Error creating delivery challan transaction:", err);
    }
  };


  const incrementDeliveryChallanNumber = (currentDeliveryChallanNumber) => {
    // Handle alphanumeric estimate numbers like "RS-00012"
    const match = currentDeliveryChallanNumber.match(/^([A-Z]+)-(\d+)$/);
    ...
    if (!isNaN(currentDeliveryChallanNumber)) {
      return parseInt(currentDeliveryChallanNumber, 10) + 1;
    }
  ...
  const handleSaveNew = async () => {
    ...
    if (!validateDeliveryChallanData()) {
    ...
    try {
      ...
      const response = await fetch(
        `${BASE_URL}/${CREATE_NEW_DELIVERY_CHALLAN}`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        console.log("Delivery Challan created successfully:", data);
        setToastMessage("Delivery Challan created successfully!");
        setToastType("success");
        setShowToast(true);

        // Also create delivery challan transaction
        await createDeliveryChallanTransaction(data);

        // Clear all form fields after successful delivery challan creation
        setPartyName("");
        setPartyPhone("");
        setPartyId(null);
        setReceivedAmount("");
        ...
        // Increment delivery challan number for next delivery challan
        setDeliveryChallanNumber((prevNumber) => incrementDeliveryChallanNumber(prevNumber));

        // Generate and download PDF after successful delivery challan creation
        // await generateAndDownloadPDF();

        // Delay navigation to allow toast to display
        setTimeout(() => {
          routerNavigate("/sales/deliverychallan/preview", {
            state: { deliveryChallanData: data },
          });
        }, 2500);
        // Add any additional logic here, like redirecting to the estimate page
      } else {
        console.error("Failed to create delivery challan");
        setToastMessage("Failed to create delivery challan. Please try again.");
        setToastType("error");
        setShowToast(true);
      }
    } catch (error) {
      ...
      setToastMessage(
        "Error creating delivery challan. Please check your connection and try again."
      );
      setToastType("error");
      setShowToast(true);
    }
  ...
  useEffect(() => {
    fetchDeliveryChallanNumber();
  }, []);

  const fetchDeliveryChallanNumber = async () => {
    try {
      const response = await fetch(
        `${BASE_URL}/${GENERATE_DELIVERY_CHALLAN_NUMBER}`
      );
      console.log(response.body);
      if (response.ok) {
        const data = await response.json();
        setDeliveryChallanNumber(data.deliveryChallanNumber);
      } else {
        console.error("Failed to fetch delivery challan number");
        setDeliveryChallanNumber("DC-00001");
      }
    } catch (error) {
      console.error("Error fetching delivery challan number:", error);
      setDeliveryChallanNumber("DC-00001");
    }
  };
  ...
  return (
    <div className="add-delivery-challan-container">
      {/* Header Section */}
      <div className="add-delivery-challan-header-section">
        <div className="add-delivery-challan-header-section-left">
          <span className="add-delivery-challan-label">
            Add Delivery Challan
          </span>
        </div>
        <div className="add-delivery-challan-header-section-right">
          <button
            className="add-delivery-challan-btn-back"
            onClick={() => {
      ...
      <div className="main-content">
        ...
        <div className="add-delivery-challan-left-column">
          <div className="add-delivery-challan-input-section">
            <div className="add-delivery-challan-customer-fields">
              <div className="add-delivery-challan-input-group">
            ...
            <div className="add-delivery-challan-address-fields">
              {showBillingAddress && (
                <div className="add-delivery-challan-address-input-group">
                  ...
                  <textarea
                    ...
                    ref={billingRef}
                  />
                  <div className="add-delivery-challan-address-actions">
                    <button
                      type="button"
                      className="add-delivery-challan-address-btn remove"
                    ...
                    <button
                      type="button"
                      className="add-delivery-challan-address-btn change"
            ...
            <div className="add-delivery-challan-date-fields">
              <div className="add-delivery-challan-input-group">
            ...
            <div className="add-delivery-challan-items-section">
              <table className="add-delivery-challan-items-table">
                <thead>
                  <tr>
                    <th className="add-delivery-challan-header-cell">#</th>
                    <th className="add-delivery-challan-header-cell">ITEM</th>
                    <th className="add-delivery-challan-header-cell">QTY</th>
                    <th className="add-delivery-challan-header-cell add-delivery-challan-price-unit-header">
                      <div className="add-delivery-challan-price-unit-split">
                        <div className="add-delivery-challan-price-label">
                          PRICE/UNIT
                        </div>
                        <div className="add-delivery-challan-tax-type-select">
                          <select
                            className="add-delivery-challan-header-tax-select"
                            value={headerTaxType}
                            onChange={(e) =>
                    ...
                    <th className="add-delivery-challan-header-cell add-delivery-challan-discount-header">
                      <div className="add-delivery-challan-discount-split">
                        <div className="add-delivery-challan-discount-label">
                          DISCOUNT (%)
                        </div>
                        <div className="add-delivery-challan-discount-sub-split">
                          <div className="add-delivery-challan-discount-percent-label">
                            %
                          </div>
                          <div className="add-delivery-challan-discount-amount-label">
                            AMOUNT
                          </div>
                        </div>
                      </div>
                    </th>
                    <th className="add-delivery-challan-header-cell add-delivery-challan-tax-header">
                      <div className="add-delivery-challan-tax-split">
                        <div className="add-delivery-challan-tax-label">
                          TAX
                        </div>
                        <div className="add-delivery-challan-tax-sub-split">
                          <div className="add-delivery-challan-tax-percent-label">
                            %
                          </div>
                          <div className="add-delivery-challan-tax-amount-label">
                            AMOUNT
                          </div>
                        </div>
                      </div>
                    </th>
                    <th className="add-delivery-challan-header-cell">
                      TOTAL
                    </th>
                  </tr>
                </thead>
                <tbody>
                  {itemInputs.map((item, index) => (
                    <tr
                      key={item.id}
                      className="add-delivery-challan-item-row"
                    >
                      <td className="add-delivery-challan-cell add-delivery-challan-item-id-cell">
                        <span className="add-delivery-challan-item-id">
                          {item.id}
                        </span>
                        {(itemInputs.length > 1 || index === 0) && (
                          <button
                            className="add-delivery-challan-remove-row-btn"
                      ...
                      <td className="add-delivery-challan-cell add-delivery-challan-item-name-cell">
                        <ItemsDropdown
                          value={item.itemName}
                          onChange={(value) =>
                      ...
                      <td className="add-delivery-challan-cell">
                        <input
                          type="number"
                          placeholder="0"
                          value={item.quantity}
                      ...
                      <td className="add-delivery-challan-cell add-delivery-challan-price-unit-cell">
                        <input
                          type="number"
                          placeholder="0.00"
                          value={item.price}
                          ...
                          className="add-delivery-challan-price-input"
                        />
                      </td>
                      <td className="add-delivery-challan-cell add-delivery-challan-discount-cell">
                        <div className="add-delivery-challan-discount-split">
                          <div className="add-delivery-challan-discount-sub-split">
                            <input
                              type="number"
                              ...
                              className="add-delivery-challan-discount-input"
                            />
                            <input
                              type="number"
                              placeholder="0.00"
                              ...
                              className="add-delivery-challan-discount-amount-input"
                            />
                          </div>
                        </div>
                      </td>
                      <td className="add-delivery-challan-cell add-delivery-challan-tax-cell">
                        <div className="add-delivery-challan-tax-split">
                          <div className="add-delivery-challan-tax-sub-split">
                            <select
                              ...
                              className="add-delivery-challan-tax-select"
                            >
                            ...
                            <input
                              ...
                              readOnly
                              className="add-delivery-challan-tax-amount-input"
                            />
                          </div>
                        </div>
                      </td>
              ...
              <div
                className="add-delivery-challan-add-row-btn"
                onClick={addNewRow}
              >
                + Add Row
              </div>
          ...
          <div className="add-delivery-challan-total-amount-bar">
        ...
        <div className="add-delivery-challan-right-column">
          <div
            className="add-delivery-challan-estimate-preview"
            ref={deliveryChallanPreviewRef}
          >
            <h2 className="add-delivery-challan-estimate-title">
              Delivery Challan
            </h2>

            <div className="add-delivery-challan-estimate-details">
              <div className="add-delivery-challan-details-header">
                <div className="add-delivery-challan-estimate-for">
                  <h4>Delivery Challan For</h4>
                  <p>{partyName || "Customer Name"}</p>
                  <p>{partyPhone || "Contact No. "}</p>
                </div>

                <div className="add-delivery-challan-estimate-info">
                  <h4>Delivery Challan Details</h4>
                  <div className="add-delivery-challan-info-row">
                    <span>Ref. No. {deliveryChallanNumber}</span>
                  </div>
                  <div className="add-delivery-challan-info-row">
              ...
              {itemInputs.some((item) => item.itemName.trim() !== "") && (
                <div className="add-delivery-challan-estimate-items-table">
                  <table className="add-delivery-challan-items-preview-table">
                    <thead>
                      <tr>
                        <th>#</th>
                        <th>Item Name</th>
                    ...
                    <tbody>
                      {itemInputs
                        ...
                        .map((item, index) => (
                          <tr
                            key={item.id}
                            className="add-delivery-challan-estimate-item-preview-row"
                          >
                            <td>{index + 1}</td>
                            <td>{item.itemName}</td>
                            ...
                            <td>
                              ...
                              <span className="add-delivery-challan-discount-percentage">
                                ({item.discount || 0}%)
                              </span>
                            </td>
                            <td>
                              ...
                              <span className="add-delivery-challan-gst-percentage">
                                (
                      ...
                      {/* Total Row */}
                      <tr className="add-delivery-challan-total-row">
                        <td colSpan="2">
                          <strong>Total</strong>
                        </td>
                        <td>
                    ...
                    <tfoot>
                      <tr>
                        <td
                          colSpan="7"
                          className="add-delivery-challan-no-padding-cell"
                        >
                          <div className="add-delivery-challan-tax-summary-wrapper">
                            <div className="add-delivery-challan-tax-summary-title">
                              Tax Summary:
                            </div>
                            <table className="add-delivery-challan-tax-summary-table">
                              <thead>
                                {(() => {
                                  ...
                                  return (
                                    <>
                                      <tr>
                                        <th
                                          rowSpan="2"
                                          className="add-delivery-challan-ts-col-hsn"
                                        >
                                          HSN/ SAC
                                        </th>
                                        <th
                                          rowSpan="2"
                                          className="add-delivery-challan-ts-col-taxable"
                                        >
                                          Taxable amount (₹)
                                        </th>
                                        {hasIGST ? (
                                          <th
                                            colSpan="2"
                                            className="add-delivery-challan-ts-col-igst"
                                          >
                                            IGST
                                          </th>
                                        ) : (
                                          <>
                                            <th
                                              colSpan="2"
                                              className="add-delivery-challan-ts-col-cgst"
                                            >
                                              CGST
                                            </th>
                                            <th
                                              colSpan="2"
                                              className="add-delivery-challan-ts-col-sgst"
                                            >
                                              SGST
                                            </th>
                                          </>
                                        ...
                                        <th
                                          rowSpan="2"
                                          className="add-delivery-challan-ts-col-total"
                                        >
                                          Total Tax (₹)
                                        </th>
                                      </tr>
                                      <tr>
                                        {hasIGST ? (
                                          <>
                                            <th className="add-delivery-challan-ts-col-rate">
                                              Rate (%)
                                            </th>
                                            <th className="add-delivery-challan-ts-col-amt">
                                              Amt (₹)
                                            </th>
                                          </>
                                        ) : (
                                      ...
                                      {taxGroupsArray.map((group, index) => {
                                        if (group.isIGST) {
                                          ...
                                          return (
                                            <tr key={index}>
                                              <td></td>
                                              <td className="add-delivery-challan-ta-right">
                                                {formatNumberWithCommas(
                                                  group.taxableAmount.toFixed(2)
                                                )}
                                              </td>
                                              <td className="add-delivery-challan-ta-center">
              ...
              <div className="add-delivery-challan-estimate-bottom-section">
                <div className="add-delivery-challan-estimate-left-column-content">
                  <div className="add-delivery-challan-amount-words">
                    <p>
                      <strong>Delivery Challan Amount In Words: </strong>{" "}
                      {numberToWords(calculateSubTotal())}
                    </p>
                  </div>
                  <div className="add-delivery-challan-terms">
                    <p>
                ...
                <div className="add-delivery-challan-right-column-content">
                  <div className="add-delivery-challan-summary-item">
                    <strong>Total</strong>
                    <span>
                      ₹ {formatNumberWithCommas(calculateSubTotal().toFixed(2))}
                    </span>
              ...
              <div className="add-delivery-challan-footer">
                <div className="add-delivery-challan-company-signature">
                  <p>For : {companyName}</p>
            ...
            <div className="add-delivery-challan-action-buttons">
              <button
                className="add-delivery-challan-save-new-btn"
                onClick={handleSaveNew}
              >
                Save & New
              </button>
              <div className="add-delivery-challan-action-icons">
                <button
                  className="add-delivery-challan-icon-btn"
